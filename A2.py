    ...
    for epoch in range(0, num_epoch):
        ...
        for i in range(10):
            error1_train, _ = model.Forward_Propagation(
                ...
                train_data[1][int((i / 10) * len(train_data[0])):int((i + 1) / 10*len(train_data[0]))], 'test')
            error01_train += error1_train
        for i in range(6):
            error1_test, _ = model.Forward_Propagation(
                test_data[0][int((i / 10) * len(test_data[0])):int(((i + 1) / 10) * len(test_data[0]))],
                test_data[1][int((i / 10) * len(test_data[0])):int(((i + 1) / 10)* len(test_data[0]))], 'test')
...
def main():
    ...
    # This shows the error rate of training and testing
